...
uses mycelium's strict window match (a window counts only when one of
its folders is the worktree itself or sits inside it), not the column's
open-or-focus one: a false "open" on a destructive prompt would cry
wolf.
